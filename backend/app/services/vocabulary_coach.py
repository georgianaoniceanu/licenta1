...
@cached("pron_emotion", ttl=3600)
def analyze_pronunciation_with_emotion(target_text: str, transcribed_text: str) -> Dict:
    """
    Analyzes pronunciation quality and detects emotional/confidence indicators.
    
    FEATURE 25 (Part 2):
    Extended pronunciation analysis that includes:
    - Emotion detection (confident, nervous, hesitant, enthusiastic)
    - Speech quality metrics (pace, clarity, rhythm)
    - Tone analysis
    - Suggestions based on emotional state
    
    Args:
        target_text: What user was supposed to say
    ...
    Returns:
        {
            ...
            "emotion_analysis": {
                ...
                "indicators": ["clear articulation", "steady pace", "natural rhythm"]
            },
            "speech_quality": {
                "pace": "good",  # slow/good/fast
                "clarity": "excellent",
                "rhythm": "natural",
                "accent_strength": "light"
            },
            "suggestions": "Practice recommendations considering emotional state"
        }
    """
    try:
        response = client.chat.completions.create(
...
Analyze pronunciation comparing target vs user speech.
Detect emotion (confident/hesitant/enthusiastic/nervous/neutral).
...
Return exactly this JSON structure:
{
    ...
    "suggestions": "<practice tips>"
                ...
                {
                    "role": "user",
                    "content": f"""Target: "{target_text}"
User said: "{transcribed_text}"

Analyze pronunciation and emotion indicators. Return ONLY JSON."""
                }
            ]
        )
        
        response_text = response.choices[0].message.content.strip()
        print(f"DEBUG: Raw Llama response: {response_text[:200]}")
        
        result = json.loads(response_text)
        print(f"✅ Analyzed pronunciation with emotion detection - Score: {result.get('pronunciation_score')}/100")
        return result
        ...
        return {
            ...
            "corrections": "Could not analyze - try again with clearer audio",
            "emotion_analysis": {
                "detected_emotion": "neutral",
                "confidence_level": "medium",
                "indicators": ["speech detected"]
            },
            "speech_quality": {
                "pace": "good",
                "clarity": "fair",
        ...
        return {
            "pronunciation_score": 65,
            "corrections": "Could not analyze - try again",
            "emotion_analysis": {
                "detected_emotion": "neutral",
                "confidence_level": "medium",
                "indicators": []
